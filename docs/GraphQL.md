...
GraphQL isn't tied to any specific database or back-end technology, instead GraphQL can be used with existing code and data.
...
GraphQL can be implemented in many programming languages as an API for _any_ back-end. Our API is just one implementation of GraphQL.

There are many things that the GraphQL specification doesn't prescribe, such as Schema design, fetching data behind the scenes, and more. These decisions are made by each individual implementation of a GraphQL server.
...
Most GraphQL implementations provide ways to register custom Scalar types.
