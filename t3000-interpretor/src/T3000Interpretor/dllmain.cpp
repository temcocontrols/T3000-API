...
encode_str* Encode_Program(char* input_text);
...
encode_str* Encode_Program( char * input_text )
{
	encode_str* encodestring = new encode_str;
	error_code = -1;
...
//fance pedit->length_code=code-pedit->code;
	...
	*pmes = 0;
	...
	memcpy(encodestring->encode_array, mycode, my_lengthcode);
	
end_encode:
	if (error_code == -1) //if -1  means no error;
	{
		encodestring->errorcode = -1;
		strcpy(encodestring->encode_message, "encode succss!");
	}
	else
	{
		// encodestring->errorcode = error;
		strcpy(encodestring->encode_message, pmes);
	}
	...
	while (tok != FINISHED)
	{
		...
		case KEYWORD:
			...
			case PRINT:
				...
				while (token_type != NL && tok != FINISHED && token_type != KEYWORD)
				{
					if (token_type != STRING)
						if (strcmp(stoupper(token), "DATE"))
							if (strcmp(stoupper(token), "PTIME"))
								if (strcmp(stoupper(token), "USER_A"))
									if (strcmp(stoupper(token), "USER_B"))
										if (strcmp(stoupper(token), "BEEP"))
											if (!define_var(token))
											{
			...
			case ALARM_AT:
				...
				if ((token_type == IDENTIFIER) && (strcmp(stoupper(token), "ALL")))
				{
					sntx_err(SYNTAX); get_nl(); break;
				};
				if (token_type == NUMBER)
		...
		if (*prog == '(')
		{
			...
			else
			{
				...
				case SENSOR_OFF:
					...
					if (*token != ')')
					{
						error_code = TRUE; *value = FALSE; sntx_err(SYNTAX); get_nl(); return;
					}
					if (type_eval)
						buf_v[index_buf - 1].op[index_op++] = ftok;
					break;
				...
				case INTERVAL:
					...
					if (*token != ')')
					{
						error_code = TRUE; *value = FALSE; get_nl(); sntx_err(SYNTAX); return;
					}
					if (type_eval)
					{
						buf_v[index_buf - 1].op[index_op++] = ftok;
	...
	case DELIMITER:
		if (*token == '\'')
		{
			*value = *prog;
			...
			if (*prog != '\'')
			{
				error_code = TRUE; *value = FALSE; sntx_err(QUOTE_EXPECTED); get_nl(); return;
			}
			prog++;
...
char* decode_point(char* token, Str_points& temp)
{
	...
	if (((q = strchr(token, '-')) != NULL) || ((q = strchr(token, '.')) != NULL))
	{
		...
		else
		{
			...
			*num_panel = atoi(pc);
			sub_panel = *num_panel;
			sprintf((char*)fance_sub_pc, "%d", (int)(*num_panel));
	...
	if ((p = strpbrk(q, "0123456789")) != NULL)
	{
		...
		if (k <= MAX_FUNCTION_COUNT)
		{
			...
			if (p == NULL)
			{
				memcpy(pmes, "error line : ", 13);
				pmes += 13;
				sprintf(pmes, "%d", lline);
				pmes += strlen(pmes);
				*pmes++ = 0x0d;
				//												fprintf(pmes,"error line %d\n",line);
				error_code = 1; return 0;
			}
			else  if
				(
					((strlen(p) == 1) && (*p == '0')) &&
			...
			else
			{
				...
				else
				{
					*num_point = atoi(p);
					...
					//sprintf(&buf[strlen(buf)], "%d", *num_panel);
					//*num_point = atoi(p);
	...
	*num_net = network;
	...
	if ((*num_panel >= 256) || b_is_instance)
	{
		if ((k != BAC_AV) && (k != BAC_AI) && (k != BAC_AO) && (k != BAC_BO) && (k != BAC_BI) && (k != BAC_BV))
		{
			...
			error_code = INSTANCE_NOT_SUPPORT;
			return 0;
		}
	}
		...
		for (j = 0; j < ptr_panel.table_bank[i]; j++)
		{
			...
			if (i != CON)
			{
				...
				if (p)
				{
					...
					{
						*var_type = LABEL_VAR;
						sprintf(token, "%d", Station_NUM);
						//Fance strcat(token,lin);
						strcat(token, "-");
						char temp[20];
						memset(temp, 0, 20);
						sprintf(temp, "%d", Station_NUM);
						strcat(token, temp);

						strcat(token, "-");
						strcat(token, ptr_panel.info[i].name);
						sprintf(&token[strlen(token)], "%d", j + 1);
						*num_point = j + 1;
						*point_type = i;
			...
			case PRINT:
				...
				while (token_type != NL && tok != FINISHED && token_type != KEYWORD)
				{
					...
					else
					{
						if (!strcmp(stoupper(token), "DATE"))
							buf_v[i].v = _DATE;
						else if (!strcmp(stoupper(token), "PTIME"))
							buf_v[i].v = PTIME;
						else	if (!strcmp(stoupper(token), "USER_A"))
							buf_v[i].v = USER_A;
						else if (!strcmp(stoupper(token), "USER_B"))
							buf_v[i].v = USER_B;
						else	if (!strcmp(stoupper(token), "BEEP"))
							buf_v[i].v = BEEP;
	...
	while (((unsigned char)*code) != 0xFE)
	{
		...
		case SET_PRINTER:
			*buf++ = *code;
			...
			break;
		case RUN_MACRO:
			sprintf(buf, "%d", (unsigned char)*code);
			buf += strlen(buf);
			code++;
			break;
		case ON:
		...
		case GOTOIF:
			...
			sprintf(buf, "%d", *((unsigned short*)&pcode[temp_n - 2 + 1]));
			buf += strlen(buf);
			code += 2;
			if (cod == GOTOIF)
				code++;    //FF
...
//									 adjustint(&n, ptrprg->type);
		...
		case CALLB:
			strcpy(buf, "PRG");
			buf += strlen("PRG");
			sprintf(buf, "%d", ((unsigned char)*code++) + 1);
			buf += strlen(buf);
			strcpy(buf, " = ");
			buf += strlen(buf);
...
end_decode:
	if (error_code != -1)
	{
		decodestr->errorcode = error_code;
...
int pointtotext(char* buf, Point_Net* point)
{
		...
		(point_type != BAC_VAR)) // 说明是新的格式，最高位用来标识.
	{
			...
			(point_type == BAC_AO) || (point_type == BAC_BO))
		{
			...
			sprintf(temp_object_instance, "%d", temp_value);
...
encode_str* Encode_function(char* input_text)
{
	init_info_table();
	Init_table_bank();
	encode_str* encodestring = Encode_Program(input_text);
